<template>
  <v-dialog v-model="active" persistent>
    <div class="service-message-wrapper primary mainBorderColor">
      <div class="close-dialog" @click="active = false">
        <img src="../../assets/img/close.svg" alt="close-icon" />
      </div>
      <h2 class="sevice-message-title textMainColor">{{ title }}</h2>
      <p class="sevice-message-description textSecondaryColor">{{ text }}</p>
      <div class="service-btn-block">
        <button 
            class="service-btn btn-cancel mainBorderColor" 
            @click="active = false">{{cancelBtnText || 'Cancel'}}
        </button>
        <button 
            class="service-btn btn-try accent" 
            @click="onConfirm">{{confirmBtnText || 'Confirm'}}
        </button>
      </div>
    </div>
  </v-dialog>
